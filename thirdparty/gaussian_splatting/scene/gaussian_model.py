...
except Exception:  # pragma: no cover - extension is built later
    distCUDA2 = None
...
class GaussianModel:
    ...
    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None):
        ...
        if depthmap is not None:
            ...
            if self.config["mapping"]["full_resolution"]:
                depthmap = F.interpolate(torch.tensor(depthmap).cuda().unsqueeze(0).unsqueeze(0),
                                         rgb_raw.shape[:2],mode='bicubic').squeeze().cpu().numpy()
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        if distCUDA2 is None:
            raise ImportError("thirdparty.simple_knn._C is not built yet.")
        dist2 = (
            torch.clamp_min(
    ...
    def load_ply(self, path):
        ...
        self.unique_kfIDs = torch.zeros((self._xyz.shape[0]), dtype=torch.int32)
        self.n_obs = torch.zeros((self._xyz.shape[0]), device="cpu").int()

    def apply_anchor_pose_update(self, kf_id, old_pose_w2c, new_pose_w2c):
        if self._xyz is None or self._xyz.shape[0] == 0:
            return 0

        if not torch.is_tensor(old_pose_w2c):
            old_pose_w2c = torch.as_tensor(old_pose_w2c, dtype=torch.float32, device=self._xyz.device)
        else:
            old_pose_w2c = old_pose_w2c.to(self._xyz.device, dtype=torch.float32)
        if not torch.is_tensor(new_pose_w2c):
            new_pose_w2c = torch.as_tensor(new_pose_w2c, dtype=torch.float32, device=self._xyz.device)
        else:
            new_pose_w2c = new_pose_w2c.to(self._xyz.device, dtype=torch.float32)

        if self.unique_kfIDs.numel() != self._xyz.shape[0]:
            return 0
        ...
        ones = torch.ones(mask.sum(), 1, device=self._xyz.device, dtype=self._xyz.dtype)
        xyz_h = torch.cat([self._xyz.data[mask], ones], dim=-1)
        updated = (transform @ xyz_h.t()).t()[:, :3]
        with torch.no_grad():
            self._xyz.data[mask] = updated
        if self.optimizer is not None:
            state = self.optimizer.state.get(self._xyz, None)
